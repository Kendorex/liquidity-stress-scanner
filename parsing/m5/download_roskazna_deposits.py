...
#
# Источник: Росказна
# Размещение средств ЕКС на банковских депозитах.
#
# Скрипт:
# 1. скачивает HTML;
...
def extract_selection_number(filename: str):
    match = re.search(r"otbor_([0-9_]+)", str(filename), flags=re.IGNORECASE)
...
def build_local_file_name(row: dict) -> str:
    ...
    return f"{date_part}_{safe_filename(row['file_name'])}"
...
def main() -> None:
    ...
    main_rows = parse_files_from_html(
        html=html,
        source_url=final_url,
        source_label="main_page",
    )
        ...
        for page in range(1, MAX_PAGES_PER_YEAR + 1):
            ...
            for row in rows:
                if pd.notna(row["date"]):
                    if pd.Timestamp(row["date"]).year != year:
                        continue
